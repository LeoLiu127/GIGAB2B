...
- 策略查找键为 `platform + market + category`，策略持久化在本机 `.runtime/template-filler-policies.sqlite3`；同类目后续上传自动复用，不需要改代码或弹窗确认。
- 策略以真实 Amazon `field_id` 保存，每条规则的动作只能是 `required`、`reminder`、`default` 或 `ignore`。`default` 必须通过当前模板下拉允许值校验。
- 系统同时保存字段 ID、Data Definitions 必填级别与允许值的模板指纹。模板变化时继续套用仍匹配的规则，并在 `policy_drift` 报告字段新增/删除、必填级别或允许值变化；漂移本身不阻断上传。
- 首次遇到没有策略的新平台/站点/类目时，返回 `policy_unconfigured` 并阻止上传。运营在页面规则编辑器保存策略后即可复用。
- 初始迁移策略：`Amazon/UK/CHAIR` 保存当前九项运营必填和 `DEFAULT` 履约渠道；`Amazon/UK/CABINET` 保存空基线策略。品牌、GTIN 豁免、原产国、危险品、电池与数量等已确认安全规则仍是平台级默认值。
